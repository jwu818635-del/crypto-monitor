...
export function DashboardContent() {
  const { reviewedRows, snapshot } = useDataApp(); const seededRows = useMemo(() => reviewedRows("market_prices", ["ticker", "date"]), [reviewedRows]);
  ...
  return <article className="crypto-dashboard"><h1 className="crypto-sr-only">加密资产技术雷达</h1>
    ...
    <section className="crypto-analysis-grid" aria-label="强弱排行与单币分析">
      ...
      <DataComponent id="crypto-alerts" queryId="watch_alerts" queryIds={["watch_alerts", "technical_signals"]} kind="custom" variant="card" title="需留意的信号" description="仅在命中既定技术规则时显示；不把任何信号视为买卖指令。" sourceRows={alerts} sourceRowsByQuery={{ watch_alerts: alerts, technical_signals: technical }} displayRows={alerts}><AlertList alerts={alerts} /></DataComponent>
    </section>
